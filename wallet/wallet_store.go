...
func (ws *walletStore) SetFeeAmount(FeeAmount int64) error {
	...
	if err != nil {
		walletlog.Error("SetFeeAmount", "marshal FeeAmount error", err)
		return types.ErrMarshal
	}

	err = ws.GetDB().SetSync(CalcWalletPassKey(), FeeAmountbytes)
	if err != nil {
		walletlog.Error("SetFeeAmount", "SetSync error", err)
		return err
	}
	return nil
}
...
func (ws *walletStore) GetFeeAmount(minFee int64) int64 {
	...
	if FeeAmountbytes == nil || err != nil {
		walletlog.Debug("GetFeeAmount", "Get from db error", err)
		return minFee
	}
	var FeeAmount int64
	err = json.Unmarshal(FeeAmountbytes, &FeeAmount)
	if err != nil {
		walletlog.Error("GetFeeAmount", "json unmarshal error", err)
		return minFee
	}
	return FeeAmount
}
...
func (ws *walletStore) SetWalletPassword(newpass string) {
	...
	if err != nil {
		walletlog.Error("SetWalletPassword", "SetSync error", err)
	}
}
...
func (ws *walletStore) GetWalletPassword() string {
	...
	if passwordbytes == nil || err != nil {
		walletlog.Error("GetWalletPassword", "Get from db error", err)
		return ""
	}
